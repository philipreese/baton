...
baton dispatch <role> [--spec <file> | --spec - | --spec-text <text>] [--room-dir <dir>] [--adapter <vendor>] [--model <m>] [--effort <e>]
                    ...
                    [--token-budget <n>] [--max-tool-steps <n>] [--verify <cmd>] [--expect-pr <true|false>] [--continue <room-dir>]
                    [--originating-pr <owner/repo#number>] [--label <text>]
...
| `--max-tool-steps <n>` | Override the dispatched role's own default cap on tool steps (#1686) for just this dispatch — the second arrest trigger alongside `--token-budget`, armed independently of it (#1682), and settling the same way: the execution is arrested mid-flight and the step goes `Indeterminate`. Role dispatch only, same refusal as `--timeout` on a template. Must be a non-negative whole number; `0` permits response-only output and requests arrest when the first tool call becomes countable. See `spec/baton.md` §3 for cap+1 and lifecycle-event details; per-role defaults are there too. |
...
| `--originating-pr <owner/repo#number>` | Give a direct follow-on role the narrow right to read and update one existing PR after Baton verifies that the canonical repository, tracked branch, open PR, and exact launch HEAD all agree, using an absolute link-free `gh` resolved outside the workspace. Queue launches also require their recorded branch to match. A queue-only recovery continuation from a failed `fix`/`continue` may instead carry the prior observed PR head and must prove the clean current workspace is that head or a descendant; the internal recovery value is never sourced from worker input or a mutable workspace probe. Baton retains the verified authority outside worker-owned room state, carries it across same-room continuation or redispatch only while the binding still agrees, and names its origin in the effective grant. Refused for templates and on any mismatch. It grants no labels, merge, enumeration, unrelated PR, or repository-setting authority; `spec/baton.md` §2 is the canonical trust contract. |
...
| `--attach <file>` | Repeatable (#1500). Copies `<file>` into the room's `artifacts/attachments/` directory before the worker starts, and appends one line to the prompt naming every attached file and that directory. Keeps a brief short instead of pasting context documents inline. Role dispatch only — refused up front on a template dispatch, the same way `--output`/`--timeout` are. Content is operator-supplied and **inbound**: it is never scanned or published as a deliverable, and an attachment is never one of the terminal step outputs exposed by the daemon projection. Each named file must exist; a missing one is a typed argument error before the room is created. |
| `--no-default-skills` | Dispatch the role without the `default_skills` its catalog entry declares (#2110), leaving only what `--skill` names. Applies to a template dispatch too. The rule, the shipped defaults, and what a redispatch inherits are in `spec/baton.md` §2, "Role default skills". |
...
`implement` declares an engine-run verify command (`pixi run gates-quiet`; in Baton's own repo a
...
— but only while they still hold the bytes AER placed: edit one and it is your work again. The rule and
what it still counts are stated once, in `spec/baton.md` §3's #1373 paragraph; the mechanism is
...
There is no pass-through rung for an existing `SKILL.md` directory (S5); the role-carried skill set
(S6) is `default_skills` (#2110, `spec/baton.md` §2), which is why a template phase now shows a
declared roster while `--skill` is still refused for a workflow template. codex receives a declared set only, never a
...
baton redispatch <room-dir> [--spec <amended-brief>] [--attach <file>]... [--adapter <vendor>] [--model <m>]
                          ...
                          [--originating-pr <owner/repo#number>] [--workstream <slug>]
```
