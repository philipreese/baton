...
|---|---|---|
| <a id="probe-2026-09-08"></a>2026-09-08, `#2124` | `claude` **2.1.263**, `agy` **1.1.27**, `codex` **0.153.2** | All 21 probe findings re-established on the same surfaces; two readings moved. `claude --help` now includes `--permission-prompt-tool` ([probe JSON, line 54](vendor-capabilities.probe.json#L54)); the 2.1.258 pin probed 2026-09-04 did not. The change occurred **between the two pins**: both a version bump and four days separate them, so this does not attribute it to 2.1.263 alone. |
| ↳ same probe | `codex` **0.153.2**, operator's account | The visible/account-sensitive `model/list` catalog dropped `gpt-5.4` (already recorded in #1875) and `gpt-5.4-mini` ([probe JSON, lines 228 and 250](vendor-capabilities.probe.json#L228)); [#2126](https://github.com/aer-works/baton/issues/2126) tracks the newly observed mini departure and the embedded recording's drift. |
...
| `--permission-prompt-tool` | **honoured** — consults a named MCP tool ([help-surface history](#probe-2026-09-08)) | **rejected**: `flags provided but not defined` |
...
The flag was undocumented in `claude --help` through 2.1.258 ([later help-surface reading](#probe-2026-09-08)),
so the original reading was *incomplete*, in the same way the `/usage` row was. What settles it is a **control flag**: pass
