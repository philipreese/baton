...
**Read this guide before developing Baton.** If your job is instead to *invoke* Baton — run a lane against
...
3. **P/Invoke Layer**: `Baton.Core`'s process execution (`BatonTask`, `src/Baton/Core/`) is plain managed code since #1474 — no FFI boundary, no native library. Four thin, internal P/Invoke surfaces remain, all under `src/Baton/Core/Internal/` and all plain Win32 rather than a vendored ABI: Windows Job Object calls (`CreateJobObject`/`AssignProcessToJobObject`/`TerminateJobObject`, `SafeJobObjectHandle.cs`) for process-tree containment; `CreateProcessW` plus its startup-attribute calls (`ContainedProcessLauncher.cs`, #2030) for the one-shot probe's atomic suspended-and-contained launch; `GlobalMemoryStatusEx` (`FreePhysicalMemory.cs`, #1934) for the conductor queue's free-memory floor; and `GetStdHandle`/`SetHandleInformation` (`StandardHandleInheritance.cs`, #2030) so nothing a lane spawns inherits Baton's own stdout/stderr. Any interaction with a vendor CLI still goes through a reviewed managed spawn seam, never a P/Invoke which selects a vendor or constructs its arguments.
...
GitHub issues in `philipreese/baton`, via `gh`. See `docs/agents/issue-tracker.md`. Branch, commit,
and PR rules stay in "Git conventions" above — that file does not restate them.

### Triage labels

The current dispatch and triage states are defined in `docs/agents/triage-labels.md`; read that
file before routing an issue.
