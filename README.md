...
| `baton queue add <tag> --role <r> --spec <f> (--issue <n> \| --workspace <d>) [--scope engine\|tooling\|docs] [--adapter] [--model] [--effort] [--skill <name>] [--timeout] [--max-tool-steps] [--token-budget] [--override-runway <reason>] [--reason <why>]` | Queue a dispatch request. `--skill` is repeatable; explicit skills are currently refused with `--lifecycle`. Copies the spec into `~/.baton/queue/specs/<tag>.md`; `--issue` provisions and trusts a worktree there and then. |
