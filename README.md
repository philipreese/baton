...
| `baton queue list [--active]` / `baton queue worktrees [--format text\|json]` / `baton queue hold` / `baton queue resume` / `baton queue import <file>` | Show each item's state, stage and room; inventory retained-worktree cleanup classification ([register](spec/baton.md#the-verbs)); pause and unpause launches; and import the pre-Baton scratchpad queue file. `--active` limits the view to queued or launched work and lifecycle terminal items that still need attention. |
