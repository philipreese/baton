...
| `baton queue add <tag> --role <r> --spec <f> (--issue <n> \| --workspace <d>) [--scope engine\|tooling\|docs] [--adapter] [--model] [--effort] [--skill <name>] [--memory-context <repository>] [--require <capability>] [--timeout] [--max-tool-steps] [--token-budget] [--override-runway <reason>] [--reason <why>]` | Queue a dispatch request. `--memory-context` creates one body-free, deterministic Baton attachment from current resolved canonical memory (fleet first); it proves neither vendor auto-ingestion nor external conductor bootstrap. `--skill` and `--require` are repeatable; requirements are compared with the effective role grant before a lane has a room or vendor process, never used to broaden that grant. `queue list` exposes declared/unknown migration coverage. Copies the spec into `~/.baton/queue/specs/<tag>.md`; `--issue` provisions and trusts a worktree there and then. |
