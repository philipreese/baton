...
| Verb | What it does |
|---|---|
| `baton run` / `baton dispatch` / `baton redispatch` | Start a workflow room, or rerun a terminal one with an amended brief. `dispatch` and `redispatch` may use `--originating-pr owner/repo#number` only to continue one open PR after Baton verifies its repository, recorded branch, and launch head with a trusted `gh`; Baton retains that authority outside worker-owned room state and exposes its origin in the effective grant. It grants no other PR or repository access. Both dispatch verbs take a repeatable `--skill <name>`, attaching a canonical skill package resolved through the rungs `spec/baton.md` §9 states — checked against the worker's grant, never widening it. |
...
| `baton ledger backfill [--dry-run] [--rooms-root <dir>] [--since <instant>]` | Recover cost-ledger rows the ledger never got: settled executions still on disk in rooms retention has not swept, and one `github-backfill` row per merged PR. Idempotent, and it only ever adds rows — `--dry-run` prints what it would write plus every room and PR it could not attribute, and why (`spec/baton.md` §7). |
| `baton ledger export --to <dir> [--as-of <yyyy-MM-dd>]` | Publish the whole cost ledger as `<dir>/<yyyy-MM-dd>.csv` and index it in that directory's `README.md`, so an efficiency analysis is reproducible from the repository rather than from one machine — the committed exports live in [`benchmarks/ledger/`](benchmarks/ledger/README.md). Read-only over the ledger, and the CSV format is redacted for it (`spec/baton.md` §7). |
| `baton audit lanes [--since <duration>] [--vendor <name>] [--format text\|json]` | Read-only audit of how much of each lane's tool-step budget bought nothing: per room and per vendor, tool calls, calls the permission grant refused, identical repeats and empty results. The drill-down behind the cost ledger's `refusedToolSteps`/`repeatedToolSteps` — `spec/baton.md` §7. |
| `baton memory audit [--repository <id>\|fleet] [--format text\|json]` | Read-only inventory of this machine's Claude memory roots (live and archived), each mapped to a canonical repository identity, with duplicate/orphan/stale/no-provenance/ambiguous findings, plus Baton's own canonical stores (the `fleet` store first) with counts only. Writes nothing — see `spec/baton.md` §12. |
| `baton memory add --text <text> --kind <kind> [--repository <id>\|fleet] [--dry-run]` | Append **one** memory to that canonical store — or, with `--repository fleet`, to the reserved **fleet** store for operator and machine facts that belong to no repository, which every repository's projection merges in ahead of its own entries (`spec/baton.md` §12 states what may be fleet-scoped and the merge rule). Otherwise this is — the **ongoing write path** (`import` below is a one-time backfill, and the vendor files are projections of what this wrote). Through the same store the importer uses, so there is no second writer. An entry byte-identical to one already stored is **refused with exit 1**, because the entry's id is derived from its subject and its text; `--dry-run` writes nothing at all and still refuses one. Every entry records who asserted it: `operator` outside a lane, `<role>/<vendor>/<room>` inside one — a reading of the lane's environment, never a credential. A successful append immediately runs the same projection as `sync --apply`; a projection failure leaves the canonical row committed and a durable retry obligation (`spec/baton.md` §12). |
| `baton memory retract <entry-id> --reason <text> [--repository <id>\|fleet]` | Append a retraction without deleting history, then run the shared projection. A missing or already-retracted id is refused; projection failure does not undo the retraction (`spec/baton.md` §12). |
| `baton memory import [--dry-run] [--root <dir>]... [--assert <path>=<repository>\|fleet]... \| --undo <manifest>` | Copy those roots' memories — plus the Codex markdown ones — into Baton's canonical per-repository store at `~/.baton/<repo-slug>/memory/entries.jsonl` (with `links.jsonl` beside it for supersession, so a link lands whichever run discovers it). Non-destructive: every source is opened read-only and left byte-identical, re-importing an unchanged tree appends nothing, and the manifest each run emits replays to a full undo — one that reversed less than its manifest claims exits non-zero and says which store file came up short. It never guesses whose memory something is: a root with no derivable repository is reported unfiled until `--assert` says otherwise, and Codex's `memories_*.sqlite` is recorded as machinery rather than read. Successful canonical changes and new assertions run the shared projection (`spec/baton.md` §12). |
| `baton memory sync [--repository <id>\|fleet] [--apply \| --check] [--format text\|json] [--repository-facts <dir>]` | Project that canonical store back into the markdown vendor memory roots as a **cache**: one Baton-owned file per root, opening with a header that names itself a cache, names the store it came from, and back-points every section to a canonical entry id. **No timestamp, on purpose** — an unchanged store projects byte-identical bytes, so a diff means the store changed. Without `--apply` nothing is written, not even a directory. Targets are discovered rather than constructed, so a repository with no matching root is reported as having none; sqlite and protobuf stores, and archived roots, are never written, and every discovered root that is not a target is listed with the reason. A checked-in repository fact outranks a vendor one of the same name, with the loser named rather than merged or dropped. **`baton memory import` skips these files rather than re-ingesting them**, so the two verbs are not a loop. **`--check` writes nothing and exits 1 when any discovered target would be rewritten**, so a CI step can gate on the projections matching the store; it is refused together with `--apply`, and a repository with no discovered root has nothing that could be rewritten, so it exits 0 rather than failing (`spec/baton.md` §12). |
...
| `baton queue list [--active]` / `baton queue worktrees [--format text\|json]` / `baton queue hold` / `baton queue resume` / `baton queue import <file>` | Show each item's state, stage and room; inventory retained worktree ownership and static cleanup refusals without changing anything; pause and unpause launches; and import the pre-Baton scratchpad queue file. `queue worktrees` is read-only, treats historical origin as unknown, and a static candidate is never deletion authorization (the register is `spec/baton.md` §13). `--active` limits the view to queued or launched work and lifecycle terminal items that still need attention. |
...
**the daemon is what starts a lane**, on its own schedule, weighing concurrency slots, free memory
against a time-of-day floor, a gap between launches and the `hold` flag, then resolving the item's
adapter/model/effort from a role-and-scope tier table. Every evaluation lands as a line in
...
(`GlassBindPolicy` states the rule and the residual; the decision is `spec/baton.md` §11 C-11). On
Windows, binding any of these prefixes can need a one-time URL reservation from an **elevated**
prompt, loopback included — `GlassHttpService`'s own remarks carry the 2026-09-08 measurement
behind that "loopback included". Grant every prefix's reservation in the same step, rather than
just the tailnet ones:

```powershell
netsh http add urlacl url=http://127.0.0.1:8420/ user=$env:USERDOMAIN\$env:USERNAME
netsh http add urlacl url=http://<your-tailscale-ip>:8420/ user=$env:USERDOMAIN\$env:USERNAME
```

One reservation per prefix, so a machine with an IPv6 tailnet address needs a third, with the
address bracketed. Don't retype any of these from here — the daemon prints the exact command,
including the loopback case, for whichever prefix it could not bind.
...
Whatever proxies in front of the daemon needs pointing at a prefix that actually got bound (the
