...
| `baton memory sync [--repository <id>\|fleet] [--apply \| --check] [--format text\|json] [--repository-facts <dir>]` | Project that canonical store into each discovered vendor memory root as a **cache**: a bounded Baton-owned section in its real index plus explicitly named per-entry detail files. Bytes outside the paired markers remain untouched; malformed markers refuse rewriting. Entries are fleet-first in stable canonical order, with title, one-line description and relative detail path; stale owned details disappear on an empty store. **No timestamp, on purpose** — unchanged input regenerates byte-identically. Without `--apply` nothing is written, not even a directory. Targets are discovered rather than constructed; sqlite/protobuf stores and archived roots are never written. **`baton memory import` skips Baton detail caches and does not re-ingest the index section**, so the two verbs are not a loop. **`--check` writes nothing and exits 1 when any discovered target would be rewritten**, so a CI step can gate on projections matching the store; it is refused together with `--apply`, and a repository with no discovered root exits 0 rather than failing (`spec/baton.md` §12). |
