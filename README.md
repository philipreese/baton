...
| `baton memory import [--dry-run] [--root <dir>]... [--assert <path>=<repository>]... \| --undo <manifest>` | Copy those roots' memories — plus the Codex markdown ones — into Baton's canonical per-repository store at `~/.baton/<repo-slug>/memory/entries.jsonl`. Non-destructive: every source is opened read-only and left byte-identical, re-importing an unchanged tree appends nothing, and the manifest each run emits replays to a full undo. It never guesses whose memory something is: a root with no derivable repository is reported unfiled until `--assert` says otherwise, and Codex's `memories_*.sqlite` is recorded as machinery rather than read (`spec/baton.md` §12). |
