...
public class VendorSpawnGateTests
{
    ...
    private static readonly Dictionary<string, string> ApprovedSpawnSites = new()
    {
        ...
        ["src/Baton.Vendors/GhPullRequestCreateProvenance.cs"] = "#2190: runs only read-only local git config/rev-parse probes through an absolute executable resolved outside the worker workspace before dispatch. It never starts gh or another vendor CLI; direct gh create remains in CodexDynamicToolPolicy behind that broker's mandatory dynamic-tool policy.",
        ["src/Baton.Cli/WorkspaceHead.cs"] = "Read-only 'git rev-parse HEAD' to capture a capture step's base ref, and (#1944) 'git rev-parse --abbrev-ref HEAD' to read the workspace's branch for the room's delivery-branch record — git, not a vendor CLI; no -p, no tool execution. Both are purely local rev-parse reads through one private spawn helper, so neither touches the network or a credential helper and the caller's token is the only bound either needs.",
        ...
        ["src/Baton.Cli/QueueWorktreeReport.cs"] = "#2318: bounded read-only 'git worktree list', 'git rev-parse', and 'git status' probes for the retained-worktree inventory — git, not a vendor CLI; no model or tool execution, no network operation, and GIT_OPTIONAL_LOCKS=0 prevents the observation from refreshing the index.",
        ["src/Baton.Cli/QueueCommand.cs"] = "The prepared Git reference fence; it uses the shared windowless start-info factory and is not a vendor dispatch path.",
        ...
        ["src/Baton.Cli/OriginatingPullRequestVerifier.cs"] = "#2178: runs only `gh pr view --json state,headRefName,headRefOid` before dispatch to bind a conductor-supplied PR to the already captured repository, branch, and launch HEAD. It is a read-only forge query, not a vendor CLI; no model or tool execution is involved.",
        ["src/Baton.Cli/Mcp/ExactFileRestoreTool.cs"] = "Spawns only bounded local git inspection/restore commands for the exact-file MCP primitive, never a vendor CLI, and uses the shared process seam with both UTF-8 streams redirected.",
