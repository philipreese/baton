...
public sealed class DispatchTemplateEndToEndTests : IDisposable
{
    ...
    public async Task Dispatching_a_template_prints_skills_for_each_worker_phase()
    {
        ...
        try
        {
            ...
            // #2110/#2132: phases whose roles carry default_skills print their declared packages.
            // This is also the roster half of that rule.
            Assert.Contains("Skills (implement, declared): baton-implement", output);
            Assert.Contains("Skills (janitor, declared): baton-janitor", output);
