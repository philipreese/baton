...
execution, the engine first validates every declared artifact already in its outbox. If all are valid,
it records the truthful cap measurement and settles that execution successfully; it neither discards the
account nor spends a checkpoint. Invalid or hollow artifacts do not qualify. Otherwise, an execution
