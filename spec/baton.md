...
In order: every line matching a known fixture-noise shape is dropped (the `CAPTURED (#1594)` marker
and the execution-stream-logger initialization warning — two unguarded `Console.Error.WriteLine`s in
Baton's own `src/` that its own test fixtures trigger, so they land in the `test-no-build` member's
captured stream and, measured on `dispatch-implement-1b79802b`/`-311af7d4`, filled the whole tail
while the failing assertion sat outside it); what survives is kept tail-first inside the shared
`MaxTailChars` budget; and any surviving test-runner failure line the cut dropped (`Failed <test>`,
`Failed!`, `Assert.`, `[FAIL]`, `error XXnnnn`) is promoted back as a short labelled excerpt above it.
**This is a display rule and only a display rule**: the whole unfiltered combined stream is
written verbatim to `VerifyRunner.RawOutputFileName` (`.verify-output.log`) in the execution's own
artifacts directory whenever verify fails, the tail names that file in its first line, and nothing
that decides an outcome reads the filtered text — `VerifyFailedKind.BuildLockBusy`'s `NotRunReason` is
parsed from the raw stream for exactly that reason. `Baton.Mutation.VerifyRunner` stays the canonical
account of the shapes themselves.
  ...
  "steps": [
    {
      ...
      "verifyTail"?: string,      // #1701/#1971: the failing gate member(s)' OWN captured output for a VerifyFailed Indeterminate, noise-filtered -- see "Engine-run verify" below for the rule and for the unfiltered file it points at. Distinct from "verify"/"verifyReason": that pair says verify never ran, this says it ran and went red.
