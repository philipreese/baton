...
**It records evaluation transitions, every launch and every launch failure — not a per-tick heartbeat.** A verdict
...
- A lane that **faults or is cancelled after launch** never reaches Terminal, so nothing writes its
  ...
  **That sentinel is projected from the room's own ledger, never fabricated.** The path is reached
  ...
  corrupt: the launcher retries projection with a short, bounded backoff before it degrades. This
  requires a holder whose sharing mode excludes readers; Baton's own live writer permits reads.
  If that hold outlasts the bound, the bare sentinel's `error` says it was held; a corrupt ledger
  names the corruption and offending line, while a missing or otherwise unreadable room record
  says that instead. The resolved queue item's error carries that same terminal error.
