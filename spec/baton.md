...
`baton dispatch` and `baton redispatch` accept one `--originating-pr owner/repo#number` option. Before a room is created, the conductor verifies that the workspace's canonical repository and recorded branch match that open PR and that its head equals the pre-dispatch workspace head. Queue launches also require the queue item's recorded branch to match the workspace branch. A lifecycle `fix` or `continue` attempt that is measured as one clean commit ahead may advance to a recovery `continue`; that queue-only launch carries the previously observed open PR head as its expected base, and the same verifier requires a clean workspace whose current HEAD is that base or its descendant. The retained value comes only from the lifecycle's prior PR observation, never worker input or a fresh mutable workspace probe; malformed, unreadable, moved, divergent, or dirty evidence refuses before spawn. Verification invokes absolute, link-free `gh` and `git` executables resolved outside the worker workspace; workspace-local executables cannot answer the ownership query. The verified value is serialized into the worker-facing binding, retained as authority in a Baton-owned record outside the room, and exposed by the effective grant together with its conductor-verified branch and head. The binding alone cannot mint authority. A same-room continuation or ordinary redispatch inherits the value only while its binding agrees with that Baton-owned record; a workspace override clears inherited evidence unless the option re-verifies it. This authority permits only the matching PR's allowed view/body operations; it never grants labels, merge, enumeration, unrelated PRs, or repository settings.
...
[docs/dispatch.md](../docs/dispatch.md) (#2010) — opt-in per call site, so a wrapper invocation that
...
```
{
  ...
  "arrests"?: [ArrestLedgerEntryView], // #1530: the room's arrest history, projected from flow.jsonl + room.jsonl by `Status.ArrestLedgerProjector` (that type's own remarks are the register for the entry shape and the two logs). Absent -- NEVER an empty array -- for a room with no `cancel.request` in its history, and absent on the terminal-sentinel fast path, which reads a frozen `terminal.json` that carries no ledger
  "arrestLedgerUnavailableReason"?: string, // #1916/#2045: present ONLY when this build could not read the room's `room.jsonl` (a version-skew `$type`), which degrades `arrests` to absent -- the same absence a room that was never arrested renders, so without this field the two are indistinguishable. Sibling key, never a field inside a half-built `arrests` array, and it carries the read exception's own message. `baton status --json` states the identical field (`WorkflowStatusView.ArrestLedgerUnavailableReason`); text mode renders it as `Arrests: ledger unavailable (<reason>)`
...
- **A room's repository identity falls back to the working directory when its recorded project root no
  ...
  tenths of the recoverable history. Every other **prose** statement of that figure — the PR body's
  findings, this file — cites this line rather than restating it; an earlier draft of this sentence said
  621 of 690 and disagreed with the command's own output (#1931 review LOW). Two things are deliberately
  outside that rule and are not drift: a **verbatim transcript** of the run, which is the measurement
  rather than a restatement of it, and a **commit body**, which is immutable and so cannot be brought
  into line by editing (#1931 re-review LOW). What the fallback trades is exactly
...
populations side by side rather than folding one into the other; it sums nothing itself. **The
correcting row's identical case above is knowingly left counted into `unread`**, and stays inside
`executions`: #1931's ruling was scoped to this population, the `--resolution none` remedy already
exists for the other, and one row kind moving without the other is what would make this register hold
two answers. Closing that half means amending the paragraph above, not this one. The drill
