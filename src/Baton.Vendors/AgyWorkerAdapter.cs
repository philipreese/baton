...
public sealed partial class AgyWorkerAdapter : IWorkerAdapter, IPermissionGrantTranslator
{
    ...
    public CoreDispatchTarget Resolve(WorkerInvocation invocation, WorkerContract contract)
    {
        ...
        return new CoreDispatchTarget(
            ...
            // #2002: agy's adapter incrementally interprets complete stream-json lines; Core receives
            // only the typed fact and never retains the vendor stream for this classification.
            DetectsOutstandingToolAtTerminalSuccess: invocation.StreamJson
                ? AgyTerminalStreamRecoveryDetector.CreateObserver()
                : null,
    ...
    {
        // #2044: SkillInlining is the one implementation both inlining vendors call. agy passes a
        // working directory, so it realizes a declared set AND the workspace scan.
        var inlined = SkillInlining.InlineSkills(promptTemplate, workingDirectory, declaredSkills);
    ...
    public async Task<WorkerCapabilities> DiscoverCapabilitiesAsync(string? workingDirectory = null, CancellationToken cancellationToken = default)
    {
        ...
        if (!string.IsNullOrWhiteSpace(workingDirectory) && Directory.Exists(workingDirectory))
        {
            ...
            // The size is what the prompt actually gains, measured on SkillInlining.InlinedSkillBody
            // rather than on the raw file, because inlining is uncapped and otherwise invisible
            // (#1929 review LOW).
