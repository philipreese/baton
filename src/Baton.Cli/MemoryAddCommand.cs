...
/// </para>
/// <para>
/// <b>It does not run <c>baton memory sync</c>.</b> The projections are stale the moment this appends,
/// and the honest handling of that is to print the exact command that refreshes them — sync writes
/// bytes into vendor memory roots, and a verb that put files there without being asked is the
/// surprising half of the pair (the same reasoning <c>MemorySyncCommand</c> gives for <c>--apply</c>
/// existing at all).
/// </para>
/// </remarks>
public static class MemoryAddCommand
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        await MemoryStore.AppendAsync([entry], entriesFile, cancellationToken).ConfigureAwait(false);
        ...
        output.WriteLine(
            "PROJECTIONS  not regenerated by this command. Refresh the vendor memory files with: " +
            $"baton memory sync --repository {repository} --apply");
