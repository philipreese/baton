...
public static class DispatchCommand
{
        ...
        IRunwayReservationPolicy? reservationPolicy = null)
    {
        ...
        // does and does not copy is that type's own doc, not restated here. Placed at the dispatch
        // rather than in QueueLauncher because the queue launches THROUGH this method, so one site
        // serves both — which does mean a hand-typed `baton dispatch` inherits on the same terms, and
        // that is the same operator decision being honoured either way.
        var inheritedCeilingFact = await InheritedProjectCeiling.TryRecordAsync(
        ...
        // Said once, in the room's own dispatch output, because a ceiling that appeared without an
        // operator typing `baton trust` is exactly the kind of widening that must not be silent.
        if (inheritedCeilingFact is not null)
        {
