...
try
{
    ...
    if (args[0] == "memory")
    {
        ...
        {
            var memoryAddOptions = MemoryAddOptionsParser.Parse(args[2..]);
            return await MemoryAddCommand
    ...
    // and nothing else -- no workflow pump, no CommandResult -- so it joins them rather than the
    // switch below. What launches a queued item is the daemon, never this verb.
    if (args[0] == "queue")
    {
